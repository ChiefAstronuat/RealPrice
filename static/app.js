...
function drawChart(points){
  ...
  const vals = [];
  points.forEach(p=>{ if(p.final!=null) vals.push(p.final); if(state.mode==='both'&&p.nowcast!=null) vals.push(p.nowcast); if(state.mode==='retail'&&p.retail!=null) vals.push(p.retail); if(state.band){vals.push(p.p10,p.p90);} });
  ...
  if(state.mode==='both') line(nowcastSeries,'#ef4444',true);
  if(state.mode==='retail') line(retailSeries,'#16a34a',false);
